...
$cond = no_contact_condition($days);
...
SELECT 
    ...
    (SELECT MAX(ct.created_at) FROM contacts ct WHERE ct.customer_id = c.id) AS last_contact_date,
    (
        SELECT ct2.next_followup
        FROM contacts ct2
        WHERE ct2.customer_id = c.id AND ct2.next_followup IS NOT NULL
...
$filter
AND $cond
ORDER BY last_contact_date ASC, c.outstanding DESC
";
...
<head>
<meta charset="UTF-8">
<title>যোগাযোগ হয়নি রিপোর্ট</title>
...
<div class="topbar">
    <div>
        <div class="topbar-title">যোগাযোগ হয়নি রিপোর্ট</div>
...
<div class="container">
    ...
    <div class="hero">
        <div>
            <h2>No Contact Customer List</h2>
            <p>যেসব গ্রাহকের গত <?php echo intval($days); ?> দিনে কোনো যোগাযোগ হয়নি অথবা কখনো যোগাযোগ হয়নি।</p>
        </div>
    ...
    <div class="box">
        <form method="GET" style="display:grid; grid-template-columns:repeat(auto-fit,minmax(170px,1fr)); gap:12px; align-items:end;">
            ...
            <div>
                ...
                <a class="btn btn-orange" href="report_no_contact.php">Reset</a>
            </div>
    ...
    <div class="cards">
        <div class="card red">
            <span>যোগাযোগ হয়নি এমন গ্রাহক</span>
            <strong><?php echo $total_count; ?></strong>
        </div>

        <div class="card">
            <span>এই গ্রাহকদের মোট বকেয়া</span>
            <strong><?php echo number_format($total_outstanding, 2); ?></strong>
        </div>

        <div class="card purple">
            <span>No Contact Threshold</span>
            <strong><?php echo intval($days); ?> Days</strong>
        </div>
    </div>
    ...
    <div class="box">
        <?php if ($total_count > 0) { ?>
            <table>
                ...
                <tr class="<?php echo (!$r['last_contact_date']) ? 'danger' : ''; ?>">
                    ...
                    <td>
                        <?php echo $r['last_contact_date'] ? htmlspecialchars($r['last_contact_date']) : '<span class="zero">কখনো যোগাযোগ হয়নি</span>'; ?>
                    </td>
                    <td>
                        <?php echo $r['next_followup_date'] ? htmlspecialchars($r['next_followup_date']) : '<span class="zero">তথ্য নেই</span>'; ?>
                    </td>
                    <td>
        ...
        <?php } else { ?>
            <p class="zero">এই scope/filter অনুযায়ী কোনো customer পাওয়া যায়নি।</p>
        <?php } ?>
    </div>
